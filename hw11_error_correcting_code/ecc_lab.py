...
def find_error(e):
    ...
    Examples:
        ...
        True
    """
    position = sum([2**(2-i) for i in e.f.keys() if e.f[i]==one])-1
    return Vec({i for i in range(7)}, {} ) if position < 0 else Vec({i for i in range(7)}, {position: one})

## Task 6
# Use the Vec class for your answers.
non_codeword = Vec({i for i in range(7)}, {0:one, 1:0, 2:one, 3:one, 4:0, 5:one, 6:one})
error_vector = Vec({i for i in range(7)}, {6:one})
...
def find_error_matrix(S):
    ...
    Example:
        ...
        True
    """
    return coldict2mat({i:find_error(mat2coldict(S)[i]) for i in mat2coldict(S).keys()})
